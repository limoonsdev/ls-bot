...
const command = new SlashCommandBuilder()
  .setName('check')
  .setDescription('🔍 Check an account with AlphaChecker')
  ...
  .addStringOption(option =>
    option.setName('combo')
      .setDescription('Combo (format: email:password)')
      .setRequired(true));

async function execute(interaction) {
  const embed = new EmbedBuilder()
    .setTitle('🚧 AlphaChecker (Coming Soon)')
    .setDescription(`> ${EMOJIS.INFO} **The advanced checking system is currently under development.**\n> It will be available very soon with premium proxies and ultra-fast validation!`)
