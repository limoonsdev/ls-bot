...
async function execute(interaction) {
  ...
  try {
    ...
    const embed = new EmbedBuilder()
      .setTitle('📦 DreamShop - Restock Effectué')
      .setDescription(
        '**Service**\n' +
...
async function handleGofileRestock(interaction, serviceId, service, gofileUrl, qualityScore) {
  ...
  try {
    const { processGofileUlp } = require('../services/gofileService');
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(service.tier === 'prime' ? '#FFD700' : COLORS.SUCCESS)
      .setFooter({ text: 'DreamShop GoFile Restock System', iconURL: PANEL_BANNER_URL })
      .setTimestamp();
